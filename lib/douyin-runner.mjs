...
async function scanVideoComments({
  ...
  for (let scrollRound = 0; scrollRound <= config.maxScrolls; scrollRound += 1) {
    ...
    for (const comment of comments) {
      ...
      const sameAsAuthor =
        comment.isAuthor ||
        normalizeName(comment.username) === normalizeName(candidate.author);
      const sameAsSelf =
        (currentUser.userId && comment.userId === currentUser.userId) ||
        (currentUser.username &&
          normalizeName(comment.username) === normalizeName(currentUser.username));
...
export async function runDouyinTask(run, storage) {
  ...
  try {
    ...
    const initialGate = await detectPageGate(page);
    if (initialGate) await handleGate(run, page, initialGate);

    let currentUser = await readCurrentUser(page, config.navigationTimeoutMs);
    if (currentUser.gate) {
      await handleGate(run, page, currentUser.gate);
      currentUser = await readCurrentUser(page, config.navigationTimeoutMs);
    }
    ...
    let search = await applySearch(page, config);
    if (search.gate) {
      await handleGate(run, page, search.gate);
      search = await applySearch(page, config);
    }

    const candidates = await collectSearchCandidates(page, config, (count) => {
    ...
    for (const candidate of selected) {
      ...
      try {
        const opened = await openCandidate(page, candidate, search.searchUrl, config);
        if (opened?.gate) {
          await handleGate(run, page, opened.gate);
          videoId = await openCandidate(page, candidate, search.searchUrl, config);
        } else {
          videoId = opened;
        }
        ...
        await processMatchedUsers({
          ...
          matches,
          run,
          sendContext: { page, config, storage, contactedUsers }
        });
        ...
        await storage.saveScreenshot(run.id, page, artifactLabel(candidate, "error"));
        run.addResult({
          videoId,
          videoTitle: candidate.title,
          videoAuthor: candidate.author,
