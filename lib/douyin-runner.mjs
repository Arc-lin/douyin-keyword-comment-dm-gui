...
export async function runDouyinTask(run, storage) {
  ...
  try {
    ...
    searchPage.setDefaultTimeout(config.actionTimeoutMs);
    searchPage.setDefaultNavigationTimeout(config.navigationTimeoutMs);
    ...
    let collectResult = await collectSearchCandidates(
      searchPage,
      config,
      (count) => {
    ...
    if (collectResult.gate) {
      await handleGate(run, searchPage, collectResult.gate);
      // 验证完成后重新搜索，再次收集
      search = await applySearch(searchPage, config);
      if (search.gate) await handleGate(run, searchPage, search.gate);
      collectResult = await collectSearchCandidates(
        searchPage,
        config,
        (count) => {
          run.updateCounts({ candidateVideos: count });
    ...
    for (const page of [videoPage, messagePage]) {
      page.setDefaultTimeout(config.actionTimeoutMs);
      page.setDefaultNavigationTimeout(config.navigationTimeoutMs);
    }
    run.log("info", "已创建独立的视频详情页和私信页");
